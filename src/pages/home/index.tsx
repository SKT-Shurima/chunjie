...
class Home extends Component<IHomeProps, IState> {
  public constructor(props: IHomeProps) {
    ...
    const arr: any[] = []
    arr.length = 12
    const data = transJsonParse(window.localStorage.getItem('data') || '[]', [])
    const subData = transJsonParse(window.localStorage.getItem('subData') || '[]', [])
    const {
      subIndex = 0,
      upColor = {
        r: 144,
  ...
  public componentDidMount() {
    window.onunload = () => {
      ...
      window.localStorage.setItem('subData', JSON.stringify(this.state.subData))
      window.localStorage.setItem(
        'otherInfo',
        JSON.stringify(
          _.pick(this.state, ['upColor', 'color', 'title', 'subTitle', 'subIndex', 'y1', 'y2'])
        )
      )
    }
  }
    ...
    return [
      {
        ...
        render: (text: any, _record: any, index: number) => (
          <Input value={text} onChange={this.handleChangeName.bind(this, rowIndex, index)} />
        )
      },
      {
        title: 'Value',
        ...
        width: '50%',
        render: (text: any, _record: any, index: number) => (
          <InputNumber value={text} onChange={this.handleChangeValue.bind(this, rowIndex, index)} />
        )
      },
      {
        title: 'Action',
  ...
  public render() {
    const { data, subData, subIndex, upColor, color, title, subTitle, y1, y2 } = this.state
    return (
        ...
        <dt className={styles.leftContainer}>
          <Row>
            <Col span={6} className={styles.labelWrapper}>
              ...
              <Input value={title} onChange={this.handleTitle.bind(this, 'title')}></Input>
            </Col>
            <Col span={6} className={styles.labelWrapper}>
              <label htmlFor="标题2">标题2</label>
              <Input value={subTitle} onChange={this.handleTitle.bind(this, 'subTitle')}></Input>
            </Col>
            <Col span={6} className={styles.labelWrapper}>
              <label htmlFor="Y1">Y1</label>
              <Input value={y1} onChange={this.handleY.bind(this, 'y1')}></Input>
            </Col>
            <Col span={6} className={styles.labelWrapper}>
              <label htmlFor="Y2">Y2</label>
              <Input value={y2} onChange={this.handleY.bind(this, 'y2')}></Input>
            </Col>
          </Row>
          <Row>
            <Col span={6} className={styles.labelWrapper}>
            ...
            <Col span={6} className={styles.labelWrapper}>
              ...
              <ColorPicker color={color} onChange={this.handleChangeColor.bind(this, 'color')} />
            </Col>
            <Col span={6} className={styles.labelWrapper}>
              <label htmlFor="次图维度">次图维度</label>
              <Select style={{ width: '50%' }} value={subIndex} onChange={this.handleSelect}>
          ...
          <Table
            ...
            pagination={false}
          />
          <Button type="dashed" onClick={this.addRow} block icon={<PlusOutlined />}>
            新增
          </Button>
        </dt>
